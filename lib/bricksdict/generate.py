...
def get_brick_matrix_smoke(face_idx_matrix, brick_shell, source_details, print_status=True, cursor_status=False):
    ...
    density_grid, flame_grid, color_grid, domain_res, max_res, adapt = get_smoke_info(source.evaluated_get(bpy.context.view_layer.depsgraph))
    brick_freq_matrix = deepcopy(face_idx_matrix)
    ...
    for x in range(len(coord_matrix)):
        for y in range(len(coord_matrix[0])):
            for z in range(len(coord_matrix[0][0])):
                ...
                bricksdict[b_key] = createBricksDictEntry(
