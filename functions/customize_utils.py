...
def merge_bricks(bricksdict, keys, cm, target_type="BRICK", any_height=False, merge_seed=None, merge_inconsistent_mats=False, direction_mult=(1, 1, 1), axis_sort_order=None, sort_fn=None):
    # initialize vars
    updated_keys = []
    brick_type = cm.brick_type
    ...
    for key in keys:
        ...
        axis_sort_order = axis_sort_order or ([2, 0, 1] if rand_state.randint(0, 2) else [2, 1, 0])
        _, new_key, _ = attempt_merge(bricksdict, key, keys, bricksdict[key]["size"], cm.zstep, brick_type, max_width, max_depth, legal_bricks_only, merge_internals_h, merge_internals_v, material_type, axis_sort_order=axis_sort_order, merge_inconsistent_mats=merge_inconsistent_mats, prefer_largest=True, direction_mult=direction_mult, merge_vertical=merge_vertical, target_type=target_type, height_3_only=height_3_only)
